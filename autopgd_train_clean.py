...
def L1_norm(x, keepdim=False):
    z = x.abs().contiguous().view(x.shape[0], -1).sum(-1)
    if keepdim:
        z = z.contiguous().view(-1, *[1]*(len(x.shape) - 1))
    return z

def L2_norm(x, keepdim=False):
    z = (x ** 2).contiguous().view(x.shape[0], -1).sum(-1).sqrt()
    if keepdim:
        z = z.contiguous().view(-1, *[1]*(len(x.shape) - 1))
    return z
...
def softloss(x: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
...
criterion_dict = {'ce': lambda x, y: F.cross_entropy(x, y, reduction='none'),'softloss':softloss,
...
def apgd_train(model, x, y, norm, eps, n_iter=10, use_rs=False, loss='ce',
    verbose=False, mixup=None, is_train=True):
    assert not model.training
    device = x.device
    ndims = len(x.shape) - 1
    
    times = {'fp': 0, 'bp': 0, 'total': time.time(), 'blk1': 0, 'blk2': 0,
        'blk3': 0}
    ...
    if verbose:
        ...
        print(' '.join([f'{k}={v:.5f} s' for k, v in times.items()]))
